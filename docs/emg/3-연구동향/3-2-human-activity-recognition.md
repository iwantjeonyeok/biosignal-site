...
(그림 3. a) Ren Y. 와 그 연구진들은 EMG와 IMU (Inertial Measurement Unit)를 통해 얻어진 데이터를 computer vision 기반 CNN (Convolution Neural Network) 알고리즘을 활용해 사람의 동작을 인식하는 연구를 수행했다 [33]. 이때 EMG 신호를 spectrogram으로 바꾸어, 그 자체를 이미지로 인식을 해 동작을 구분하는 피처로 사용했다.
