...
| Pham (2023) [^3] | 대규모 MI-EEG 데이터셋 기반 웨이블릿 특징 융합 분류 성능 향상 | Wavelet Scattering Features + SVM + Fuzzy Rule-based Classifier Fusion | 웨이블릿-시간 특성과 반복성 플롯을 기반으로 두 개 분류기(SVM 등)를 훈련시킨 후 fuzzy rule 기반 융합 시스템으로 최종 분류.  | | Pham (2023) [^3] | 대규모 MI-EEG 데이터셋 기반 웨이블릿 특징 융합 분류 성능 향상 | Wavelet Scattering Features + SVM + Fuzzy Rule-based Classifier Fusion | 웨이블릿-시간 특성과 반복성 플롯을 기반으로 두 개 분류기(SVM 등)를 훈련시킨 후 fuzzy rule 기반 융합 시스템으로 최종 분류.  | |
# 4. References
