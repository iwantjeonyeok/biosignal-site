...
MIT-BIH Long-Term ECG Database는 2개의 ECG 리드로 구성된 장시간 심전도 데이터셋으로, 128Hz의 고정 샘플링 주파수로 약 14~22시간 동안 수집된 ECG 신호를 포함하고 있습니다. 이 데이터셋은 정상 박동을 중심으로 심실성 조기수축, 상심실성 조기박동, 융합박동, 신호 품질 변화 등 다양한 박동 라벨을 포함하고 있으며, 전체 라벨 통계에서는 정상 박동이 대부분을 차지합니다. 또한 장기 ECG 분류, 적응형 QRS 검출, 박동별 파형 분류, ECG 분절화 및 실시간 심장 모니터링과 같은 연구에서 장시간 심전도 신호를 분석하기 위한 데이터셋으로 활용됩니다.
