...
| --- | --- | --- | --- | --- |
| 8 | 60 | 100 | 24 | (EEG).txt, (impedances-before, after).txt, (conductor).txt, (digitizer).bvct, (joints).txt |
...
| Wang (2024) [3] | 범용 EEG 표현 학습을 위한 대규모 self-supervised pretraining | Transformer | Depthwise separable convolution을 기반으로 한 경량 CNN 아키텍처를 통해 다양한 BCI 패러다임(P300, MRCP, ERN, SMR)에 대해 적은 데이터로도 높은 성능을 달성. EEGNet은 해석 가능한 EEG feature를 학습. |
