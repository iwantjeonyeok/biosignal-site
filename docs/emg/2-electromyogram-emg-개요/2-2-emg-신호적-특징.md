...
이외에도 EMG 계측시에 영향을 주는 노이즈들은 다음 **표 1**과 같다. PLI (Power Line Interference) 는 전력선에서 발생하는 전자기적 간섭에 의해 발생하는 노이즈로, 50Hz 또는 60Hz의 주파수대에서 나타난다. 또한, 고조파(Harmonics)의 형태로도 나타나, PLH (Power Line Harmonics) 각 기본주파수의 자연수배의 영역(100Hz, 150Hz, 200Hz)에서도 노이즈가 나타난다. HFN (High Frequency Noise)는 주변 장치에서 발생하는 고주파 전자기파에서 발생되는 노이즈이다. 이외에도, MA (Motion Artifacts)가 존재하며, 이는 사람의 움직임에 따라 발생하는 노이즈이다. 여기서 언급한 노이즈들은 대부분의 환경에서 공통적으로 발견되는 노이즈로, 각 실험환경에 따라 다른 노이즈들이 관찰되기도 한다.
...
![각 EMG 연구별 전처리 적용 예시](./image-2.png)

**표 2.** 각 EMG 연구별 전처리 적용 예시
