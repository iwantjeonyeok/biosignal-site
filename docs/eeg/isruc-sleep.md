...
![image.png](isruc-sleep/image.png)
...
| 
Wang (2024) [^3]         
   | 시공간 다변량 시계열(MTS) 데이터의 복합 의존성 모델링   | Fully-Connected Spatial-Temporal GNN (FC-STGCN) | 각 시점마다 센서 간 spatial graph를 구성하고, 모든 시점 간 센서 쌍의 연결을 포함하는 fully-connected edge를 통해 temporal dependency까지 포함된 그래프를 생성. FC-GCN과 이동 window 기반 GNN layer로 시간-공간 상의 모든 상호작용(dependency)을 통합적으로 학습함.  |
